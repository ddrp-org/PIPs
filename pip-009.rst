...
#. ``title``: ``string``
#. ``body``: ``string``
#. ``reference``: ``string | RecordID`` (see `PIP-1`_)
...
- if it is a ``RecordID`` that reference another ``PST`` record, this record SHOULD be treated as a reply to that post.
- if it is a ``RecordID`` that reference another ``PST-C`` record, this record SHOULD be treated as a post into that channel.
- if it is a ``RecordID`` that reference an unknown record, this record SHOULD be treated as an original post without topic.
...
- if it is a ``RecordID`` that reference another ``PST`` record, this record SHOULD be treated as a reply to that post.
- if it is a ``RecordID`` that reference another ``PST-C`` record, this record SHOULD be treated as a sub-channel under that channel (sub-forums, .
- if it is a ``RecordID`` that reference an unknown record, this record SHOULD be treated as a channel with no topic.
...
- if it is a ``RecordID`` that reference a ``PST`` record, this record SHOULD be treated as a moderation toward that post.
- if it is a ``RecordID`` that reference an unknown record, this record SHOULD be ignored.
...
- if it is a ``timestamp`` that does not reference another ``MOD`` record, this record SHOULD be ignored.
...
Record Fields
*************
#. ``reference``: ``DomainName | RecordID`` (see `PIP-1`_)

The ``reference`` can reference multiple things:

- if it is a ``RecordID`` that reference a ``PST`` record, this record SHOULD be treated as a connection (subscribe, mute) to that post.
- if it is a ``RecordID`` that reference an unknown record, this record SHOULD be ignored.
- if it is a ``DomainName`` that reference an known name, this record SHOULD be treated as a connection to that name.
...
Record Fields
*************
#. ``name``: ``DomainName`` (see `PIP-1`_)
#. ``reference``: ``RecordID`` (see `PIP-1`_)
   
The ``name`` reference a valid ``DomainName``. This record SHOULD be treated as a link to that name's moderation records.

The ``reference`` can reference multiple things:

- if it is blank, this record SHOULD be treated as a link to all of that name's moderations records.
- if it is a ``RecordID`` that reference a ``PST`` record, this record SHOULD be treated as a link to that name's moderations records under a specific post.
- if it is a ``RecordID`` that reference an unknown record, this record SHOULD be ignored.
...
- if it is a ``timestamp`` that does not reference another ``CNT`` record, this record SHOULD be ignored.
